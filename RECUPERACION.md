...
- Commit `6758a59` publicado en main: supervisor y regresiones; escaneo de secretos de ese lote exit 0. Documentación y estado factual preparados para commit propio. En aquella tanda no se ejecutaron modelos reales ni se activó bucle de producto adversarial.
- Continuación autorizada posterior: publicación MIT pública en `6adff1d`, security reporting privado habilitado, Actions sigue desactivado. Presupuesto posterior fijo20:17–21:47UTC en CONTINUACION.md; no reiniciar deadlines históricos.
- Construcción posterior: UnixReceiver autentica UID del kernel, dos canales con métodos separados; DurableController registra/reserva antes del único envío y reconcilia UNKNOWN sólo mediante recibo. Consulta de recibos scopeada al principal externo. Commit de código y pruebas `8e9f652`.
- Cuatro agentes utilizados: dos constructores iniciales, revisor readonly y constructor de dos fixes. Revisor detectó efecto no probado por comprobar sólo versión y posible huérfano tras dockercreate ambiguo. Rojo/verde nuevos, confirmación de horario exacto y cleanup por identidad propia verificada; coordinador ejecutó kernel/SQLite/Docker realmente.
- Demo Unix/SQLite: UNKNOWN→CONFIRMED sin reenvío, un cambio exacto y revocación REJECTED; mismo UID y aprobaciones sintéticas, nunca humano autenticado. Preflight separado:24/24 con UIDs25001/25002/25003, snapshot público RO intacto y contenedor propio eliminado. C1-T02/VM dedicada/contención adversarial siguen falsos.
- Suite final402/402 sin skips: macOS fuente8.39s, macOS sdist/entorno limpio8.95s, Linux aarch6417.43s. Se descubrió/controló omisión de CONTROL.es.md en sdist; regresión añadida. Instalación offline falló por caché incompleta, no por el paquete: sync normal de dependencias públicas fijadas enuv.lock resolvió el prerrequisito. Ambas demos del wheel corrieron fuera de la fuente. Evidencia temporal `limite-distribution-evidence-bsl549xq`.
- Primera suite Linux terminó FAILED en importación de Hypothesis nativo desde tmpfs noexec; un probe separado reprodujo mmap EPERM. Segunda corrida sólo habilitó exec en workspace de código confiable del runner de tests:402passed. No se alteraron pruebas ni el perfil noexec del preflight de permisos. Contenedores propios retirados; logs conservados en runs/linux-suite{,-exec}-20260919.
- Dos humos MLX actuales, mismas5referencias públicas, perfiles ya existentes, sin entrenamiento ni pesos nuevos: base0/5 y cobertura4/5 (FAILED por formato incompleto), typed3/5 cobertura5/5 (FINISHED, todavía2errores). 8.46s/6.34s internos y picos2.59/2.71GB; límite90s por corrida y8GiB de asignador. Huella de referencias igual. No se sustituyen por evidencia reservada ni mejora general. Los procesos propios ya no existen. Ver runs/model{,-typed}-revalidation-20260919; no publicar pesos, manifiestos privados ni logs incidentalmente.
